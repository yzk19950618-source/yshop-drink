					...
					<view class="left overflow-hidden" v-else>
						<view class="store-name" @click="selectShop()">
							<view>{{ store.name }}
								...
								<text class="small" v-else-if="orderType == 'takeout'">(外卖订单)</text>
							</view>
							<view class="iconfont iconarrow-right"></view>
						</view>
					</view>
...
const toPay = () => {
	...
	if (!isLogin.value) {
		...
		// distance>0 时表示配置了配送半径；为 0 时不校验半径，仅下外卖类型订单
		if (orderType.value == 'takeout' && store.value.distance > 0 && store.value.distance < store.value.far) {
...
const saveCartToStorage = () => {
	const payload = JSON.parse(JSON.stringify(cart.value || []))
	if (!payload.length) {
