	...
	<view class="container position-relative">
		...
		<view style="z-index: 1;"
			...
			<view class="bg-primary h-100 d-flex align-items-center just-content-center text-color-white font-size-base"
				style="padding: 0 60rpx;" @tap="debounce(submit, 500)">提交订单</view>
		</view>
		<!-- 付款栏 end -->
...
const amount = computed(() =>{
	...
	// 加配送费（外卖订单；distance 为 0 时仍可按门店配置的 deliveryPrice 计费）
...
const pay = async() => {
	try {
		...
		const oid = encodeURIComponent(order.orderId)
		const pt = encodeURIComponent(payType.value)
		const amt = encodeURIComponent(amount.value)
		uToast.value.show({
			message: '正在跳转收银台',
			type: 'success'
		})
		uni.navigateTo({
			url: `/pages/components/pages/pay/order-pay?orderId=${oid}&payType=${pt}&amount=${amt}`
		})
	} catch (e) {
		const backendMsg = e?.data?.msg || e?.msg || '提交订单失败，请稍后重试'
		uToast.value.show({
			message: backendMsg,
			type: 'error'
		});
